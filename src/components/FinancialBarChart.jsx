  ...
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block", overflow: "visible" }}>
      {hasRevenue && <>
        ...
        <text x={padL + 13} y={16} fill={theme.textSecondary} fontSize="9" fontFamily="'Inter',sans-serif">Przychody</text>
      </>}
      {hasNetIncome && <>
        <rect x={padL + 82} y={8} width={9} height={9} fill="#00c896" opacity="0.85" rx="2" />
        <text x={padL + 95} y={16} fill={theme.textSecondary} fontSize="9" fontFamily="'Inter',sans-serif">Zysk netto</text>
      </>}
      {data.map((d, i) => {
        ...
        return (
          <g key={d.year}>
            {d.revenue !== null && hasRevenue && <>
              <rect x={cx - barW - gap} y={padT + chartH - revH} width={barW} height={revH} fill="#58a6ff" opacity="0.85" rx="2" />
              <text x={cx - barW / 2 - gap} y={padT + chartH - revH - 4} textAnchor="middle" fill={theme.textSecondary} fontSize="7" fontFamily="'Inter',sans-serif">{fmtVal(d.revenue)}</text>
            </>}
            {d.netIncome !== null && hasNetIncome && <>
              <rect x={cx + gap} y={padT + chartH - netH} width={barW} height={netH} fill={netColor} opacity="0.85" rx="2" />
              <text x={cx + barW / 2 + gap} y={padT + chartH - netH - 4} textAnchor="middle" fill={theme.textSecondary} fontSize="7" fontFamily="'Inter',sans-serif">{fmtVal(d.netIncome)}</text>
            </>}
            <text x={cx} y={H - 6} textAnchor="middle" fill={theme.textSecondary} fontSize="10" fontFamily="'Inter',sans-serif">{d.year}</text>
          </g>
        );
      })}
    </svg>
