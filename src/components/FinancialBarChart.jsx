        ...
        return (
          <g key={d.year}>
            {d.revenue !== null && hasRevenue && <>
              ...
              <text x={cx - barW / 2 - gap} y={padT + chartH - revH - 4} textAnchor="middle" fill={theme.textSecondary} fontSize="7" fontFamily="'Space Grotesk',sans-serif">{fmtVal(d.revenue)}</text>
            </>}
            {d.netIncome !== null && hasNetIncome && <>
              <rect x={cx + gap} y={padT + chartH - netH} width={barW} height={netH} fill={netColor} opacity="0.85" rx="2" />
              <text x={cx + barW / 2 + gap} y={padT + chartH - netH - 4} textAnchor="middle" fill={theme.textSecondary} fontSize="7" fontFamily="'Space Grotesk',sans-serif">{fmtVal(d.netIncome)}</text>
            </>}
            <text x={cx} y={H - 6} textAnchor="middle" fill={theme.textSecondary} fontSize="10" fontFamily="'Space Grotesk',sans-serif">{d.year}</text>
          </g>
        );
      })}
    </svg>
