...
const CATEGORIES = [
  {
    ...
    label: "Podstawy",
    icon: "📚",
    description: "Dowiedz się jak działa giełda, jak otworzyć konto maklerskie i zacząć swoją przygodę z inwestowaniem.",
    color: "#58a6ff",
  },
  {
    key: "analiza",
    label: "Analiza",
    icon: "📊",
    description: "Naucz się czytać wykresy, wskaźniki fundamentalne i oceniać wartość spółek.",
    color: "#00c896",
  },
  {
    key: "strategia",
    label: "Strategia",
    icon: "♟️",
    description: "Poznaj sprawdzone strategie inwestycyjne: dywidendową, value investing, momentum i inne.",
    color: "#f0883e",
  },
];
    ...
    <div style={{ minHeight: "100vh", background: theme.bgPage, color: theme.text, fontFamily: "'Inter', sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: isMobile ? "16px 16px" : "24px 24px" }}>
        ...
        <Breadcrumbs
          theme={theme}
          items={[
            { label: "Strona główna", href: "/", onClick: onBack },
            { label: "Edukacja" },
          ]}
        />
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: isMobile ? 28 : 42, fontWeight: 900, color: theme.textBright, margin: "0 0 16px", lineHeight: 1.2 }}>
            Naucz się inwestować na GPW
          </h1>
          <p style={{ fontSize: isMobile ? 15 : 17, color: theme.textSecondary, maxWidth: 580, margin: "0 auto 28px", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            {[{ label: "10 artykułów", icon: "📄" }, { label: "3 kategorie", icon: "🗂️" }, { label: "Bezpłatnie", icon: "✅" }].map(item => (
              <span key={item.label} style={{ background: theme.bgCardAlt, border: `1px solid ${theme.border}`, borderRadius: 8, padding: "6px 14px", fontSize: 13, color: theme.textMuted, display: "flex", alignItems: "center", gap: 6 }}>
        ...
        {/* Category cards */}
        <h2 style={{ fontSize: 20, fontWeight: 800, color: theme.textBright, marginBottom: 16 }}>Kategorie</h2>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 16, marginBottom: 48 }}>
          {CATEGORIES.map(cat => (
            ...
            >
              ...
              <p style={{ margin: "0 0 16px", fontSize: 14, color: theme.textSecondary, lineHeight: 1.6 }}>{cat.description}</p>
              <span style={{ fontSize: 13, color: cat.color, fontWeight: 600 }}>
                {getArticlesByCategory(cat.key).length} artykuł{getArticlesByCategory(cat.key).length === 1 ? "" : "ów"} →
              </span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 280px", gap: 32 }}>
          {/* Articles grid */}
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: theme.textBright, margin: 0 }}>Wszystkie artykuły</h2>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {popularArticles.map((article, i) => (
                    ...
                    >
                      ...
                      <div>
                        <div style={{ fontSize: 13, color: theme.text, fontWeight: 600, lineHeight: 1.4 }}
              ...
              <div style={{ background: `linear-gradient(135deg, ${theme.accent}18 0%, ${theme.accent}08 100%)`, border: `1px solid ${theme.accent}30`, borderRadius: 14, padding: 20 }}>
                ...
                <input
                  ...
                  style={{
                    ...
                    background: theme.bgCard, border: `1px solid ${theme.borderInput}`,
                    borderRadius: 8, color: theme.text, fontFamily: "inherit", fontSize: 13, marginBottom: 8,
                  }}
                />
                <button style={{
                  width: "100%", padding: "10px", background: theme.accent, color: "#000",
                  border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", fontSize: 13,
                }}>
        ...
        {/* Mobile newsletter */}
        {isMobile && (
          <div style={{ background: `linear-gradient(135deg, ${theme.accent}18 0%, ${theme.accent}08 100%)`, border: `1px solid ${theme.accent}30`, borderRadius: 14, padding: 20, marginTop: 32 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: theme.textBright, marginBottom: 6 }}>📬 Newsletter WIGmarkets</div>
            <input type="email" placeholder="Twój adres email" style={{ width: "100%", boxSizing: "border-box", padding: "9px 12px", background: theme.bgCard, border: `1px solid ${theme.borderInput}`, borderRadius: 8, color: theme.text, fontFamily: "inherit", fontSize: 13, marginBottom: 8 }} />
            <button style={{ width: "100%", padding: "10px", background: theme.accent, color: "#000", border: "none", borderRadius: 8, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", fontSize: 13 }}>Zapisz się</button>
          </div>
        )}
